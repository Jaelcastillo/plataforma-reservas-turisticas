...
@media (max-width: 850px) {
    ...
    .auth-left { min-height: 320px; padding: 2rem; }
}
"""
...
def registro() -> rx.Component:
    return rx.box(
        rx.html(f"<style>{REGISTRO_CSS}</style>"),
        rx.box(
            rx.box(
                rx.box(
                    rx.html('<div class="auth-logo">Travel<span>World</span></div>'),
                    rx.html(
                        '<p class="auth-quote">Crea tu cuenta<br>'
                        'y comienza tu<br>próxima aventura.</p>'
                        '<p class="auth-quote-sub">Experiencias premium, reservas seguras y destinos inolvidables.</p>'
            ...
            rx.box(
                rx.vstack(
                    rx.vstack(
                        ...
                        rx.text(
                            "Comienza a planificar experiencias inolvidables",
                            font_size="1rem",
                            color="#5C4E3A",
                            text_align="center",
                        ),
                    ...
                    rx.vstack(
                        _field("👤", "Nombre completo", AuthState.nombre, AuthState.set_nombre),
                        ...
                        _field("🔐", "Confirmar contraseña", AuthState.confirmar_password, AuthState.set_confirmar_password, "password"),
                        spacing="3",
                        width="100%",
                    ),
                    rx.cond(
                    ...
                    rx.html(
                        ...
                        '<span style="font-size:.78rem;color:#A99882;">¿Ya tienes cuenta?</span>'
                        '<div class="auth-divider-line"></div>'
                        '</div>'
                    ),
                    rx.link(
                        rx.text(
                            "¿Ya tienes cuenta? ",
                            rx.text.span("Inicia sesión", color=GOLD, font_weight="900", text_decoration="underline"),
                            color=TEXT_S,
                            text_align="center",
                        ),
                        href="/login",
